...
// Create tables
db.exec(`
  DROP TABLE IF EXISTS award_deals;
  DROP TABLE IF EXISTS points_programs;

  CREATE TABLE award_deals (
  ...
  CREATE INDEX idx_deals_origin ON award_deals(origin);
  CREATE INDEX idx_deals_destination ON award_deals(destination);
  CREATE INDEX idx_deals_program ON award_deals(airline_program);
  CREATE INDEX idx_deals_cpp ON award_deals(cents_per_point);
  CREATE INDEX idx_deals_date ON award_deals(departure_date);
  CREATE INDEX idx_deals_created ON award_deals(created_at);
...
const insertProgram = db.prepare(
  `INSERT INTO points_programs (name, transfer_partners) VALUES (?, ?)`
);

insertProgram.run(
  "Chase Ultimate Rewards",
  JSON.stringify(["United MileagePlus", "Flying Blue", "British Airways Avios", "Southwest Rapid Rewards", "Singapore KrisFlyer"])
);
insertProgram.run(
  "Amex Membership Rewards",
  JSON.stringify(["Delta SkyMiles", "Flying Blue", "British Airways Avios", "ANA Mileage Club", "Singapore KrisFlyer", "Aeroplan"])
);
insertProgram.run(
  "Capital One Miles",
  JSON.stringify(["Flying Blue", "British Airways Avios", "Turkish Miles&Smiles", "Avianca LifeMiles", "Singapore KrisFlyer"])
);

// Seed award deals — realistic mock data
interface DealSeed {
  origin: string;
  destination: string;
  origin_city: string;
...
const deals: DealSeed[] = [
  ...
  // ANA Mileage Club — famous sweet spot
  ...
  // Some lower-value deals (should be filtered out at 1.8 cpp threshold)
...
const insertMany = db.transaction((rows: DealSeed[]) => {
  for (const d of rows) {
    const cpp = Math.round(((d.cash_price_usd * 100) / d.points_required) * 100) / 100;
    insertDeal.run(
      d.origin, d.destination, d.origin_city, d.destination_city,
      d.airline_program, d.cabin_class, d.points_required, d.cash_price_usd,
      cpp, d.departure_date, d.return_date, d.is_round_trip
    );
  }
});
